<template>
  ...
  <URange v-model="distance" name="distance" :min="0" :max="30" />
...
const showMarkerbox = (event, item) => {
  // if (event.originalEvent.srcElement.classList.contains('markerbox') && item.isBig !== false) {
  //   // goToSinglePost(item);
  // } else {
...
const showMarkerbox2 = (event, item) => {
  event.originalEvent.srcElement.parentElement.children[1]?.classList.toggle('show_markerbox');
  event.originalEvent.srcElement.parentElement.parentElement.classList.toggle('show_markerbox');
  console.log(event.originalEvent.srcElement.parentElement.parentElement);
  console.log(event.originalEvent.srcElement.classList);
  console.log(event.originalEvent.srcElement.classList.contains('markerbox'));
  if (event.originalEvent.srcElement.classList.contains('markerbox') && item.isBig !== false) {
    goToSinglePost(item);
  } else if (event.originalEvent.srcElement.classList.contains('q-icon')) {
    if (!item.alwaysHideMapHint) {
      event.originalEvent.srcElement.parentElement.children[1]?.classList.toggle('show_markerbox');
    } else if (item.isBig !== false) {
      goToSinglePost(item);
    }
  }
};
